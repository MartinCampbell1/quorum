...
interface CustomToolFormProps {
  onAdd: (tool: { key: string; name: string; description: string; tool_type: string; config: Record<string, string> }) => void;
  onCancel: () => void;
}
...
export function CustomToolForm({ onAdd, onCancel }: CustomToolFormProps) {
  const [toolType, setToolType] = useState<string>("http_api");
    ...
    <Card className="py-0 border-dashed border-foreground/20">
      <CardContent className="px-4 py-3.5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onCancel} className="p-1 rounded-md text-muted-foreground hover:text-foreground cursor-pointer">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="flex gap-2">
          ...
          />
          <Select value={toolType} onValueChange={(v) => { if (v !== null) setToolType(v); }}>
        ...
        <Button onClick={handleSubmit} disabled={!name.trim()} size="sm" className="w-full text-xs">
